<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_yunshanfu:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_yunshanfu:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_yunshanfu:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/yunshanfu.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="账号信息" :show-overflow-tooltip="true" width="280">
				<template slot-scope="scope">
					<span>云闪付账号：{{scope.row.other_info}}</span><br>
					<span>开户名：{{scope.row.name}}</span>
				</template>
			</el-table-column>
			...
			<el-table-column label="单笔控制" width="160">
				<template slot-scope="scope">
					<span>{{scope.row.min_amount}} - {{scope.row.max_amount}}</span>
				</template>
			</el-table-column>
			<el-table-column label="设置监控" :show-overflow-tooltip="true" width="150">
				<template slot-scope="scope">
					<span>额度：
			...
			<el-table-column label="操作" width="180" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_yunshanfu:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_yunshanfu:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :title="title" :visible.sync="open" width="600px" append-to-body :close-on-click-modal="false">
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				<el-row>
					...
					<el-form-item label="云闪付账号">
						<el-input v-model="form.other_info" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
...
<script>
export default {
	name: "channel_list_yunshanfu", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 107,
	...
	methods: {
		...
		handleExport() {
			...
			that.exportExcel("channel_yunshanfu/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_yunshanfu')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_yunshanfu', JSON.stringify(auto_reload))
