<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_alipay_transfer:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_alipay_transfer:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_alipay_transfer:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/alipay_transfer.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="卡商" prop="card_business_realname" :show-overflow-tooltip="true" width="70" />
			<el-table-column label="支付宝信息" :show-overflow-tooltip="true" width="150">
			...
			<el-table-column label="设置监控" :show-overflow-tooltip="true" width="130">
				<template slot-scope="scope">
					<span>额度：
			...
			<el-table-column label="操作" width="330" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_alipay_transfer:save:edit']" plain>编辑</el-button>
					<el-button type="primary" :disabled="scope.row.is_online == 1" class="tb-button" icon="el-icon-setting" @click="handleEditInfo(scope.row)" v-hasPermi="['channel_alipay_transfer:save_info:edit']" plain>设置</el-button>
					...
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_alipay_transfer:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog title="设置" :visible.sync="open_info" width="600px" append-to-body :close-on-click-modal="false">
			<el-form ref="form_info" :model="form_info" :rules="rules_info" label-width="100px">
				<el-row>
					<el-form-item label="">
						请登录<span class="text-danger bolder">{{form_info.name}}</span>账号获取密钥
					</el-form-item>
		...
		<el-dialog title="登录" :visible.sync="open_login" width="400px" append-to-body :close-on-click-modal="false" :show-close="false">
			<div class="login">
				<div class="title">
					请使用<span class="text-danger bolder">{{form_login.name}}</span>账号扫码登录
				</div>
				<div class="qrcode">
...
<script>
export default {
	name: "channel_list_alipay_transfer", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 105,
	...
	methods: {
		...
		getList() {
			...
			that.loading = true;
			that.request({
				url: "channel_alipay_transfer/list",
		...
		handleEdit(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_transfer/view",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		handleEditInfo(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_transfer/view_info",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		submitForm: function() {
			...
			that.$refs["form"].validate(valid => {
				if (valid) {
					that.request({
						url: "channel_alipay_transfer/save",
						data: that.form,
					}).then(res => {
						that.open = false;
						that.getList();
		...
		submitFormInfo: function() {
			...
			that.$refs["form_info"].validate(valid => {
				if (valid) {
					that.loadingShow();
					that.request({
						url: "channel_alipay_transfer/save_info",
						data: that.form_info,
					}).then(res => {
						that.loadingClose();
		...
		handleDelete(row) {
			...
			that.$confirm("是否确认删除?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer/delete",
					data: {
						ids,
					},
				}).then(res => {
		...
		enable(row) {
			...
			that.$confirm("是否确认开启此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer/enable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_alipay_transfer/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_alipay_transfer')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_alipay_transfer', JSON.stringify(auto_reload))
