<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_alipay_bank:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_alipay_bank:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_alipay_bank:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/alipay_bank.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" width="200" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_alipay_bank:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_alipay_bank:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
			...
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				<el-row>
					...
					<!-- <el-form-item label="银行支行">
						<el-input v-model="form.bank_branch" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item> -->
...
<script>
export default {
	name: "channel_list_alipay_bank", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 109,
	...
	methods: {
		...
		handleDelete(row) {
			...
			that.$confirm("是否确认删除?", "提示", {
				...
				that.request({
					url: "channel_alipay_bank/delete",
					data: {
						ids,
					},
				}).then(res => {
		...
		enable(row) {
			...
			that.$confirm("是否确认开启此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_bank/enable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_bank/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_alipay_bank/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_alipay_bank')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_alipay_bank', JSON.stringify(auto_reload))
