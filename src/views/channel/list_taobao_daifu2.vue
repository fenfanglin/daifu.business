<template>
	...
	<div v-else class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
			...
			<el-form-item class="mr10">
				<el-input v-model="queryParams.product_account" placeholder="淘宝订单" clearable/>
			</el-form-item>
			<el-form-item class="mr10">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_taobao_daifu2:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_taobao_daifu2:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_taobao_daifu2:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/taobao_daifu2.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="卡商" prop="card_business_realname" :show-overflow-tooltip="true" width="120" />
			...
			<el-table-column label="二维码" :show-overflow-tooltip="true" width="200">
				<template slot-scope="scope">
					<span v-if="scope.row.type == 1" class="pointer text-success" @click="listAmount(scope.row)">
						剩余：{{scope.row.product_ready}}<br>
						总数：{{scope.row.product_total}}
					</span>
				</template>
			</el-table-column>
			<el-table-column label="设置监控" :show-overflow-tooltip="true" width="200">
				<template slot-scope="scope">
					<span>额度：
			...
			<el-table-column label="操作" width="250" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_taobao_daifu2:save:edit']" plain>编辑</el-button>
					<!-- <el-button type="primary" :disabled="scope.row.is_online == 1" class="tb-button" icon="el-icon-setting" @click="handleEditInfo(scope.row)" v-hasPermi="['channel_taobao_daifu2:save_info:edit']" plain>设置</el-button>
					...
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_taobao_daifu2:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :title="title2" :visible.sync="open2" width="800px" append-to-body :close-on-click-modal="false" :before-close="closeAmount">
...
<script>
import ChannelProduct3 from './components/ChannelProduct3_2.vue'

export default {
	name: "channel_list_taobao_daifu2", // 路由上 '/' 换成 '_'
	components: {
		ChannelProduct3,
	},
	data() {
	...
	methods: {
		...
		getList() {
			...
			that.loading = true;
			that.request({
				url: "channel_taobao_daifu2/list",
		...
		handleEdit(row) {
			...
			that.reset();
			that.request({
				url: "channel_taobao_daifu2/view",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		handleEditInfo(row) {
			...
			that.reset();
			that.request({
				url: "channel_taobao_daifu2/view_info",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		submitForm: function() {
			...
			that.$refs["form"].validate(valid => {
				if (valid) {
					that.request({
						url: "channel_taobao_daifu2/save",
						data: that.form,
					}).then(res => {
						that.open = false;
						that.getList();
		...
		submitFormInfo: function() {
			...
			that.$refs["form_info"].validate(valid => {
				if (valid) {
					that.loadingShow();
					that.request({
						url: "channel_taobao_daifu2/save_info",
						data: that.form_info,
					}).then(res => {
						that.loadingClose();
		...
		handleDelete(row) {
			...
			that.$confirm("是否确认删除?", "提示", {
				...
				that.request({
					url: "channel_taobao_daifu2/delete",
					data: {
						ids,
					},
				}).then(res => {
		...
		enable(row) {
			...
			that.$confirm("是否确认开启此数据?", "提示", {
				...
				that.loadingShow();
				that.request({
					url: "channel_taobao_daifu2/enable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_taobao_daifu2/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_taobao_daifu2/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_taobao_daifu2')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_taobao_daifu2', JSON.stringify(auto_reload))
