<template>
	...
	<div v-else class="app-container">
		...
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/rmb.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
				...
				<el-row>
					...
					<el-form-item label="类型">
						<el-radio-group v-model="form.type">
							<el-radio v-for="item in rmbType" :label="item.value">{{item.label}}</el-radio>
						</el-radio-group>
					</el-form-item>
					<el-form-item v-if="form.type == 1" label="钱包编号">
						<el-input v-model="form.account" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
					<el-form-item v-else label="手机号">
						<el-input v-model="form.account" maxlength="11" show-word-limit autocomplete="off" />
					</el-form-item>
...
export default {
	...
	computed: {
		...
		rmbType() {
			return this.$store.getters.rmbType
		},
	},
	created() {
		let that = this
	...
	methods: {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_rmb')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_rmb', JSON.stringify(auto_reload))
