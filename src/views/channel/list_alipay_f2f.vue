<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_alipay_f2f:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_alipay_f2f:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_alipay_f2f:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/alipay_f2f.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" width="250" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_alipay_f2f:save:edit']" plain>编辑</el-button>
					<el-button type="primary" class="tb-button" icon="el-icon-setting" @click="handleEditInfo(scope.row)" v-hasPermi="['channel_alipay_f2f:save_info:edit']" plain>设置</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_alipay_f2f:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
export default {
	name: "channel_list_alipay_f2f", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 114,
	...
	methods: {
		...
		handleEditInfo(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_f2f/view_info",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		submitFormInfo: function() {
			...
			that.$refs["form_info"].validate(valid => {
				if (valid) {
					that.request({
						url: "channel_alipay_f2f/save_info",
						data: that.form_info,
					}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_f2f/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_alipay_f2f/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_alipay_f2f')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_alipay_f2f', JSON.stringify(auto_reload))
