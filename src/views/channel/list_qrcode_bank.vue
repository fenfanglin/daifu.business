	...
	<div v-else class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
			<el-form-item class="mr10">
				<el-input v-model="queryParams.keyword" placeholder="银行卡开户名/银行卡号" clearable/>
			</el-form-item>
			<el-form-item class="mr10">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_qrcode_bank:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_qrcode_bank:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_qrcode_bank:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/qrcode_bank.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="卡商" prop="card_business_realname" :show-overflow-tooltip="true" width="90" />
			<el-table-column label="开户名" prop="name" :show-overflow-tooltip="true" width="100" />
			<el-table-column label="银行信息" :show-overflow-tooltip="true" width="250">
				<template slot-scope="scope">
					<span>名称：{{scope.row.system_bank_name}}</span><br>
					<span>卡号：{{scope.row.account}}</span>
				</template>
			</el-table-column>
			<el-table-column label="单笔控制" width="150">
				<template slot-scope="scope">
					<span>{{scope.row.min_amount}} - {{scope.row.max_amount}}</span>
				</template>
			</el-table-column>
			<el-table-column label="设置监控" :show-overflow-tooltip="true" width="160">
				<template slot-scope="scope">
					<span>额度：
			...
			<el-table-column label="银行卡余额" width="120">
				<template slot-scope="scope">
			...
			<el-table-column label="订单统计" :show-overflow-tooltip="true" width="350">
				<template slot-scope="scope">
					<span>今日：{{scope.row.today_success_amount}}元，{{scope.row.today_success_amount}}/{{scope.row.today_success_amount}}笔，{{scope.row.today_success_amount}}%</span><br>
					<span>昨日：{{scope.row.yesterday_success_amount}}元，{{scope.row.yesterday_success_amount}}/{{scope.row.yesterday_success_amount}}笔，{{scope.row.yesterday_success_amount}}%</span>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" width="200" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_qrcode_bank:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_qrcode_bank:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
			...
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				<el-row>
					...
					<el-form-item label="银行">
						<el-select v-model="form.system_bank_id" filterable>
					...
					<el-form-item label="费率">
						<el-input-number v-model="form.rate" :controls="false" class="w210px text-left" />
						<span class="ml10 text-danger">（千分之三填0.003）</span>
					</el-form-item>
					...
					<el-form-item label="设备id">
						<el-input v-model="form.device_imei" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
					<el-form-item label="微信号">
						<el-input v-model="form.device_no" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
...
<script>
export default {
	name: "channel_list_qrcode_bank", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 125,
	...
	methods: {
		...
		handleDelete(row) {
			...
			that.$confirm("是否确认删除?", "提示", {
				...
				that.request({
					url: "channel_qrcode_bank/delete",
					data: {
						ids,
					},
				}).then(res => {
		...
		enable(row) {
			...
			that.$confirm("是否确认开启此数据?", "提示", {
				...
				that.request({
					url: "channel_qrcode_bank/enable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_qrcode_bank/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_qrcode_bank/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_qrcode_bank')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_qrcode_bank', JSON.stringify(auto_reload))
