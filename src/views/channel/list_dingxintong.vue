	...
	<div v-else class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
			<el-form-item class="mr10">
				<el-input v-model="queryParams.keyword" placeholder="商户ID/APPID" clearable />
			</el-form-item>
			<el-form-item class="mr10">
			...
			<el-form-item>
				...
				<el-dropdown split-button :type="auto_reload.class" class="ml10" @command="command => handleAutoReloadChange(command)">
					...
					<el-dropdown-menu slot="dropdown">
						<el-dropdown-item v-for="item in autoReloadTimer" :command="item.value">{{ item.label }}</el-dropdown-item>
					</el-dropdown-menu>
				</el-dropdown>
			</el-form-item>
		</el-form>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" width="180" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_dingxintong:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_dingxintong:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :title="title" :visible.sync="open" width="600px" append-to-body :close-on-click-modal="false">
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				<el-row>
					<el-form-item v-if="user.type == 1" label="工作室">
						<el-select v-model="form.card_business_id">
							<el-option v-for="item in optionCardBusiness" :key="item.id" :label="item.name" :value="item.id" />
						</el-select>
					</el-form-item>
					<el-form-item label="商户ID">
						<el-input v-model="form.mchid" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
					<el-form-item label="APPID">
						<el-input v-model="form.appid" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
					<el-form-item label="密钥ID">
						<el-input v-model="form.key_id" maxlength="50" show-word-limit autocomplete="off" />
					</el-form-item>
					<el-form-item label="密钥">
						<el-input v-model="form.key_secret" maxlength="255" show-word-limit autocomplete="off" />
					</el-form-item>
...
<script>
export default {
	name: 'channel_list_dingxintong', // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 2,
	...
	computed: {
		...
		autoReloadTimer() {
			return this.$store.getters.autoReloadTimer
		}
	},
	created() {
		let that = this
	...
	methods: {
		...
		handleDelete(row) {
			...
			that.$confirm('是否确认删除?', '提示', {
				...
				.then(function () {
					that.request({
						url: 'channel_dingxintong/delete',
						data: {
							ids
						}
					}).then(res => {
		...
		enable(row) {
			...
			that.$confirm('是否确认开启此数据?', '提示', {
				...
				.then(function () {
					that.request({
						url: 'channel_dingxintong/enable',
						data: {
							no: row.no
						}
					})
		...
		disable(row) {
			...
			that.$confirm('是否确认关闭此数据?', '提示', {
				...
				.then(function () {
					that.request({
						url: 'channel_dingxintong/disable',
						data: {
							no: row.no
						}
					})
