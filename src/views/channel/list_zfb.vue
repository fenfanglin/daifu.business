<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_zfb:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_zfb:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_zfb:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/zfb.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="账号名称" prop="name" :show-overflow-tooltip="true" width="120" />
			<el-table-column label="支付宝收款码" prop="account" />
			<el-table-column label="单笔控制" width="150">
				<template slot-scope="scope">
					<span>{{scope.row.min_amount}} - {{scope.row.max_amount}}</span>
				</template>
			</el-table-column>
			<el-table-column label="监控额度" width="120">
				<template slot-scope="scope">
					<span v-if="scope.row.check_quota == 1">{{scope.row.quota}}</span>
					<span v-else class="text-danger">关闭</span>
				</template>
			</el-table-column>
			<el-table-column label="监控笔数" width="120">
				<template slot-scope="scope">
			...
			<el-table-column label="操作" width="200" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_zfb:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_zfb:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
export default {
	name: "channel_list_zfb", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 104,
	...
	methods: {
		...
		handleExport() {
			...
			that.exportExcel("channel_zfb/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_zfb')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_zfb', JSON.stringify(auto_reload))
