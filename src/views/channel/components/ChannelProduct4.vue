<template>
	<div class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_product:save4:add']">新增</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-upload2" @click="open2 = true" v-hasPermi="['channel_product:import4']">导入</el-button>
			</el-col>
		</el-row>

		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			<el-table-column label="商品ID" prop="account" width="150" />
			<el-table-column label="商品名称" prop="name" />
			<el-table-column label="金额" prop="amount" width="100" />
			<el-table-column label="操作" width="140" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" @click="handleEdit(scope.row)" v-hasPermi="['channel_product:save4:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" @click="handleDelete(scope.row)" v-hasPermi="['channel_product:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog title="导入" :visible.sync="open2" width="1000px" height="800px" append-to-body :close-on-click-modal="false" :before-close="closeImport">
			<channel-product4-import :data-no="dataNo" v-if="open2" @closeImport="closeImport" />
		</el-dialog>
	</div>
</template>

<script>
import ChannelProduct4Import from './ChannelProduct4Import.vue'

export default {
	components: {
		ChannelProduct4Import,
	},
	props: {
		dataNo: {
			type: String,
