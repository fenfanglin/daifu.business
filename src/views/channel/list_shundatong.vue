	...
	<div v-else class="app-container">
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" width="180" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_shundatong:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_shundatong:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
export default {
	name: 'channel_list_shundatong', // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 1,
	...
	methods: {
		...
		disable(row) {
			...
			that.$confirm('是否确认关闭此数据?', '提示', {
				...
				.then(function () {
					that.request({
						url: 'channel_shundatong/disable',
						data: {
							no: row.no
						}
					})
