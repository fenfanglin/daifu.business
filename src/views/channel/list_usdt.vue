	...
	<div v-else class="app-container">
		<el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
			<el-form-item class="mr10">
				<el-input v-model="queryParams.keyword" placeholder="钱包名称/TRC20地址" clearable/>
			</el-form-item>
			<el-form-item class="mr10">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_usdt:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_usdt:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_usdt:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/usdt.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row>
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="账号信息" :show-overflow-tooltip="true">
				<template slot-scope="scope">
					<span>钱包名称：{{scope.row.name}}</span><br>
					<span>TRC20地址：{{scope.row.account}}</span>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" width="180" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_usdt:save:edit']" plain>编辑</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_usdt:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
...
<script>
export default {
	name: "channel_list_usdt", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 102,
	...
	methods: {
		...
		handleExport() {
			...
			that.exportExcel("channel_usdt/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_usdt')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_usdt', JSON.stringify(auto_reload))
