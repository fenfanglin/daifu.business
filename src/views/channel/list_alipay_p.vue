<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_alipay_p:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_alipay_p:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_alipay_p:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/alipay_p.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="支付宝信息" :show-overflow-tooltip="true" width="220">
				<template slot-scope="scope">
					<span>名称：{{scope.row.name}}</span><br>
					<span>APPID：{{scope.row.account}}</span>
				</template>
			</el-table-column>
			...
			<el-table-column label="操作" width="250" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_alipay_p:save:edit']" plain>编辑</el-button>
					<el-button type="primary" :disabled="scope.row.is_online == 1" class="tb-button" icon="el-icon-setting" @click="handleEditInfo(scope.row)" v-hasPermi="['channel_alipay_p:save_info:edit']" plain>设置</el-button>
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_alipay_p:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :title="title" :visible.sync="open" width="600px" append-to-body :close-on-click-modal="false">
			<el-form ref="form" :model="form" :rules="rules" label-width="100px">
				<el-row>
					...
					<el-form-item v-if="form.is_special_business" label="类型">
						<el-radio-group v-model="form.type">
							<el-radio v-for="item in alipayPType" :label="item.value">{{item.label}}</el-radio>
						</el-radio-group>
					</el-form-item>
					<el-form-item v-if="form.type == 2" label="回调地址">
						<el-input v-model="form.notify_url" autocomplete="off" />
					</el-form-item>
		...
		<el-dialog :title="title2" :visible.sync="open2" width="1200px" append-to-body :close-on-click-modal="false" :before-close="closeAmount">
			<channel-product :data-no="data_no" v-if="open2" />
		</el-dialog>
	</div>
</template>

<script>
import ChannelProduct from './components/ChannelProduct.vue'

export default {
	name: "channel_list_alipay_p", // 路由上 '/' 换成 '_'
	components: {
		ChannelProduct,
	},
	data() {
	...
	methods: {
		...
		handleEditInfo(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_p/view_info",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		submitFormInfo: function() {
			...
			that.$refs["form_info"].validate(valid => {
				if (valid) {
					that.loadingShow();
					that.request({
						url: "channel_alipay_p/save_info",
						data: that.form_info,
					}).then(res => {
						that.loadingClose();
		...
		handleExport() {
			...
			that.exportExcel("channel_alipay_p/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_alipay_p')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_alipay_p', JSON.stringify(auto_reload))
