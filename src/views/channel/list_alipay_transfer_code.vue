<template>
	...
	<div v-else class="app-container">
		...
		<el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-plus" @click="handleAdd" v-hasPermi="['channel_alipay_transfer_code:save:add']">新增</el-button>
			</el-col>
			<!-- <el-col :span="1.5">
				<el-button type="danger" icon="el-icon-delete" :disabled="multiple" @click="handleDelete" v-hasPermi="['channel_alipay_transfer_code:delete']">删除</el-button>
			</el-col> -->
			<el-col :span="1.5">
				<el-button type="success" icon="el-icon-full-screen" @click="handleScanCode" v-hasPermi="['channel_alipay_transfer_code:scaned_code']">扫码领取</el-button>
			</el-col>
			<el-col :span="1.5">
				<el-button type="primary" icon="el-icon-download" @click="handleExport" v-hasPermi="['channel_alipay_transfer_code:export']">导出</el-button>
			</el-col>
		</el-row>
		
		<!-- <el-row :gutter="10" class="mb20">
			<el-col :span="1.5">
				<a href="https://jqkstore.oss-cn-hongkong.aliyuncs.com/document/alipay_transfer_code.zip" class="download_document">
					下载教程
				</a>
			</el-col>
		</el-row> -->
		...
		<el-table ref="multipleTable" v-loading="loading" border empty-text="没有相关数据" :data="list" @selection-change="handleSelectionChange">
			...
			<el-table-column label="卡商" prop="card_business_realname" :show-overflow-tooltip="true" width="100" />
			<el-table-column label="支付宝信息" :show-overflow-tooltip="true" width="240">
				<template slot-scope="scope">
					<span>名称：{{scope.row.name}}</span><br>
					<span>UID：{{scope.row.account}}</span>
				</template>
			</el-table-column>
			<el-table-column label="账号余额" width="140">
				<template slot-scope="scope">
			...
			<el-table-column label="最后在线时间" prop="last_online_time" width="200" />
			...
			<el-table-column label="操作" width="330" fixed="right">
				<template slot-scope="scope">
					<el-button type="primary" class="tb-button" icon="el-icon-edit" @click="handleEdit(scope.row)" v-hasPermi="['channel_alipay_transfer_code:save:edit']" plain>编辑</el-button>
					<el-button type="primary" :disabled="scope.row.is_online == 1" class="tb-button" icon="el-icon-setting" @click="handleEditInfo(scope.row)" v-hasPermi="['channel_alipay_transfer_code:save_info:edit']" plain>设置</el-button>
					...
					<el-button type="danger" class="tb-button" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['channel_alipay_transfer_code:delete']" plain>删除</el-button>
				</template>
			</el-table-column>
		</el-table>
			...
			<el-form ref="form_info" :model="form_info" :rules="rules_info" label-width="100px">
				<el-row>
					<el-form-item label="支付宝账号">
						{{form_info.account}}（{{form_info.name}}）
					</el-form-item>
...
<script>
export default {
	name: "channel_list_alipay_transfer_code", // 路由上 '/' 换成 '_'
	data() {
		return {
			user: {},
			channel_id: 116,
	...
	methods: {
		...
		getList() {
			...
			that.loading = true;
			that.request({
				url: "channel_alipay_transfer_code/list",
		...
		handleEdit(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_transfer_code/view",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		handleEditInfo(row) {
			...
			that.reset();
			that.request({
				url: "channel_alipay_transfer_code/view_info",
				data: {
					no: row.no,
				},
			}).then(res => {
		...
		submitForm: function() {
			...
			that.$refs["form"].validate(valid => {
				if (valid) {
					that.request({
						url: "channel_alipay_transfer_code/save",
						data: that.form,
					}).then(res => {
						that.open = false;
						that.getList();
		...
		submitFormInfo: function() {
			...
			that.$refs["form_info"].validate(valid => {
				if (valid) {
					that.loadingShow();
					that.request({
						url: "channel_alipay_transfer_code/save_info",
						data: that.form_info,
					}).then(res => {
						that.loadingClose();
		...
		handleDelete(row) {
			...
			that.$confirm("是否确认删除?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer_code/delete",
					data: {
						ids,
					},
				}).then(res => {
		...
		enable(row) {
			...
			that.$confirm("是否确认开启此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer_code/enable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		disable(row) {
			...
			that.$confirm("是否确认关闭此数据?", "提示", {
				...
				that.request({
					url: "channel_alipay_transfer_code/disable",
					data: {
						no: row.no,
					},
				}).then(res => {
		...
		handleExport() {
			...
			that.exportExcel("channel_alipay_transfer_code/export", this.queryParams, "管理员").then(res => {
		...
		autoReload() {
			...
			let auto_reload = that.cookies.get('auto_reload_alipay_transfer_code')
			if (auto_reload) {
		...
		handleAutoReloadChange(command) {
			...
			this.cookies.set('auto_reload_alipay_transfer_code', JSON.stringify(auto_reload))
		...
		handleScanCode() {
			...
			let href = '/channel/list_alipay_transfer_code_scan'
